...
void OnroadWindow::offroadTransition(bool offroad) {
  ...
  nvg->setStreamType(wide_cam ? VISION_STREAM_WIDE_ROAD : VISION_STREAM_RGB_ROAD);
...
void OnroadHud::paintEvent(QPaintEvent *event) {
  ...
  if (engageable) {
    drawIconRotate(p, x, y, engage_img, iconbgColor, 1.0); // wheel rotate
  }
  ...
  y = radius * 2.5 + (bdr_s * 2);
  y = radius * 2.5 + (bdr_s * 3) + 10;
  drawLeftDevUi(p, x, y);
  p.setOpacity(1.0);
...
void OnroadHud::drawLeftDevUi(QPainter &p, int x, int y) {
  ...
  if (engageable) {
    ...
    float desire_angle = cc.getActuators().getSteeringAngleDeg();
    valueColor = limeColor;
    snprintf(val_str, sizeof(val_str), "%.0f%s", desire_angle, "°");
...
void NvgWindow::drawSpeed(QPainter &p) {
  UIState *s = uiState();
  const SubMaster &sm = *(s->sm);
  float current_speed = std::max(0.0, sm["carState"].getCarState().getCluSpeedMs() * (s->scene.is_metric ? MS_TO_KPH : MS_TO_MPH));
  auto car_state = sm["carState"].getCarState();
  ...
  QString speed;
  speed.sprintf("%.0f", current_speed);
...
void NvgWindow::drawMaxSpeed(QPainter &p) {
  ...
  // kph
  float cruiseMaxSpeed = sm["controlsState"].getControlsState().getVCruise();

  const bool is_cruise_set = cruiseMaxSpeed != 0 && cruiseMaxSpeed != 255;
  ...
  // Max Speed
  {
    ...
    if(is_cruise_set && !s->scene.is_metric) { cruiseMaxSpeed *= KM_TO_MILE; }
    ...
    speed_rect.moveTop(max_speed_rect.top() + 30);
    ...
    if (is_cruise_set) {
      ...
      configFont(p, "Inter", 64, "sans-semibold");
      str = "N/A";
    }
    speed_rect.moveCenter({max_speed_rect.center().x(), 0});
    speed_rect.moveTop(max_speed_rect.top() + 120);
    p.drawText(speed_rect, Qt::AlignCenter | Qt::AlignVCenter, str);
  }
