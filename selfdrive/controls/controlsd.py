...
# bellows are for Slow on Curve by Neokii
  ...
  def state_transition(self, CS):
    ...
    apply_limit_speed, road_limit_speed, left_dist, first_started, limit_log = \
      SpeedLimiter.instance().get_max_speed(CS, self.v_cruise_kph, self.autoNaviSpeedCtrlStart, self.autoNaviSpeedCtrlEnd)
...
# 2 lines for Slow on Curve
  ...
  def state_control(self, CS):
    ...
    if not self.joystick_mode:
      ...
      actuators.accel = self.LoC.update(CC.longActive and CS.cruiseState.enabled, CS, long_plan, pid_accel_limits, t_since_plan)
