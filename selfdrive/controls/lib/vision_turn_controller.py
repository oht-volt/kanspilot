...
class VisionTurnController():
  ...
  def get_speed_scale(self, road_rank):
    if road_rank == 0:
      return [self._SPEED_SCALE_BP, [1.0, 1.0]]
    elif road_rank in [10, 20, 30]:
      return [self._SPEED_SCALE_BP, [0.9, 1.0]]
    elif road_rank in [1,11,21,31]:
      return [self._SPEED_SCALE_BP, [0.85, 1.0]]
    else:
      return [self._SPEED_SCALE_DEFAULT_BP, [1.05, 1.0]]
  ...
  def _update_tw_params(self):
    self._a_target.update_alpha(0.3)
    lat_accel_factor = 1.0
    long_accel_factor = 1.0
