...
_TURNING_LAT_ACC_TH = 1.3  # Lat Acc threshold to trigger turning turn state. //default 1.6 : not working, =< 1.4 : working
...
# Lookup table for the minimum smooth deceleration during the ENTERING state
# depending on the actual maximum absolute lateral acceleration predicted on the turn ahead.
_ENTERING_SMOOTH_DECEL_V = [-0.4, -1.5]  # min decel value allowed on ENTERING state
_ENTERING_SMOOTH_DECEL_BP = [1., 3.]  # absolute value of lat acc ahead

# Lookup table for the acceleration for the TURNING state
# depending on the current lateral acceleration of the vehicle.
_TURNING_ACC_V = [0.6, 0.4, -0.3]  # acc value
